...
def extract_feature(dataset, model_path, num_classes, model_name = 'I3D', num_frames = 16, modality = 'rgb', save_dir = '.', root_dir = '.',
    ...
    if model_name == 'I3D':
        ...
        print('We only implemented I3D and C3D models.')
    ...
    train_dataloader = DataLoader(VideoDataset(dataset=dataset, split='train',clip_len=num_frames, resize_height=resize_height, resize_width=resize_width, crop_size = crop_size,
                                               label_path = label_path, root_dir = root_dir, output_dir = save_dir), batch_size=batch_size, shuffle=True, num_workers=num_worker)
    val_dataloader   = DataLoader(VideoDataset(dataset=dataset, split='val',  clip_len=num_frames, resize_height=resize_height, resize_width=resize_width, crop_size = crop_size, 
                                               label_path = label_path, root_dir = root_dir, output_dir = save_dir), batch_size=batch_size, num_workers=num_worker)
        ...
        for inputs, label in tqdm(loader):
            ...
            if t > 1600:    # Only extract features for 1600 frames when it is more than 1600 frames.
                for start in range(1, t-56, 1600):
                    ...
                    if model_name == 'I3D':
                        output = output.squeeze(2).squeeze(2).squeeze(2).data.cpu().numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_dir',      type = str, default = './dataloader', help = "The path of label.")
    ...
    num_classes = class_dict.get(args.dataset, 0)
    if num_classes == 0:
        if not args.n_class:
